...
Build a discrete masked diffusion model that learns the distribution of boulder problems on LED climbing training boards and generates novel climbs conditioned on difficulty grade, wall angle, quality, and classic status. The system supports **multiple boards** — Tension Board 2, Kilter Board, and MoonBoard — via a shared model architecture with board-specific vocabularies and embeddings. Each board uses the same Aurora Climbing SQLite schema, the same `frames` encoding, and the same training pipeline, differing only in hold placements, coordinate geometry, and role definitions.

### Supported Boards

| Board | BoardLib name | Adjustable angle | Grade range | Database size (est.) | Notes |
|---|---|---|---|---|---|
| **Tension Board 2** | `tension` | 0–65° | V0–V14+ | ~20K–80K climbs | Mirror + spray layouts; wood + dual-tex holds |
| **Kilter Board** | `kilter` | 0–70° | V0–V14+ | ~160K+ climbs | Largest database; Original + HW Fullride layouts |
| **MoonBoard** | `moonboard` | Fixed 25° or 40° | V3/V4–V14+ | ~50K–100K climbs | Multiple hold sets (2016, 2017, 2019, 2024); smallest/most crimpy holds |
...
#### 1.5.5 Layout and Hold Set Selection

Each board supports multiple layouts and hold sets. A model is trained per (board, layout, hold_set) combination:

| Board | Common layout choice | Hold set notes |
|---|---|---|
| Tension TB2 | Mirror (12×12) | Single hold set covers both mirror and spray |
| Kilter | Original (8×12 or 12×12) | Single hold set; also HW Fullride layout |
| MoonBoard | Standard (40°) | Multiple sets: 2016, 2017, 2019, 2024 — each has different placements |

The config file specifies which `layout_id` and `set_id` to use. The pipeline automatically resolves valid placements for the chosen combination via `product_sizes_layouts_sets`.
...
```python
{
    ...
    "vocab_meta": {                        # for reconstruction (board-specific)
        ...
        "V_pos": int,                      # number of valid placements
...
y = MLP_combine(cat([diff_embed, angle_embed, classic_embed, quality_embed]))  # (B, d_model)
```

Inject `y` the same way as `t` (add to AdaLN, or prepend as a conditioning token at position 0).

For boards with a fixed angle (e.g., MoonBoard at 40°), the angle MLP is omitted and its slot in the concatenation is filled with zeros. This is controlled by the `angle_conditioning: true/false` flag in the board config.
...
2. At each step, going from `t` to `t - 1/T`:
   a. Run the model to get logits at all MASK positions.
   b. Apply CFG.
   c. Compute the number of tokens to unmask at this step: `n_unmask = round(k · (γ(t) - γ(t - 1/T)))`.
   d. **Confidence-based selection:** rank MASK positions by their max predicted probability. Unmask the top `n_unmask` positions.
   e. For each selected position: sample a token from the categorical distribution (with optional temperature).
   f. Replace the MASK at those positions with the sampled tokens. Fix them for all subsequent steps.
3. At `t = 0`, all `k` positions have been assigned tokens.
...
6. **Valid placements only:** guaranteed by the vocabulary (each token maps to a valid placement for the target board/layout).
...
```bash
# Generate 10 Kilter Board problems at V6, 40 degrees
python generate.py --board kilter --difficulty 16 --angle 40 --n 10

# Generate MoonBoard problems (angle auto-detected from config)
python generate.py --board moonboard --difficulty 18 --n 10

# Generate Tension Board classics
python generate.py --board tension --difficulty 14 --angle 40 --classic --n 10
```
